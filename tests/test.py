...
class UtilsTest(unittest.TestCase):
    def test_extract_id_from_url(self):
        url_id_list = [
            # New-style url
            ('https://docs.google.com/contacts/d/'
...
class gcontactTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        try:
            ...
            cls.gc = gcontact.Book(auth=OAuthCredentialObject)
        except IOError as e:
            msg = "Can't find %s for reading test configuration. "
            raise Exception(msg % e.filename)

    def setUp(self):
        self.assertTrue(isinstance(self.gc, gcontact.Book))


class BookTest(gcontactTest):

    """Test for gcontact.client."""

    def test_open(self):
        title = self.config.get('Contact', 'title')
        contact = self.gc.open(title)
        self.assertTrue(isinstance(contact, gcontact.Contact))

    def test_no_found_exeption(self):
        noexistent_title = "Please don't use this phrase as a name of a sheet."
        self.assertRaises(gcontact.ContactNotFound,
                          self.gc.open,
                          noexistent_title)

    def test_open_by_key(self):
        key = self.config.get('Contact', 'key')
        contact = self.gc.open_by_key(key)
        self.assertTrue(isinstance(contact, gcontact.Contact))

    def test_open_by_url(self):
        url = self.config.get('Contact', 'url')
        contact = self.gc.open_by_url(url)
        self.assertTrue(isinstance(contact, gcontact.Contact))

    def test_openall(self):
        contact_list = self.gc.openall()
        for s in contact_list:
            self.assertTrue(isinstance(s, gcontact.Contact))

    def test_create(self):
        title = gen_value('TestContact')
        new_contact = self.gc.create(title)
        self.assertTrue(isinstance(new_contact, gcontact.Contact))


class ContactTest(gcontactTest):

    """Test for gcontact.Contact."""

    def setUp(self):
        super(ContactTest, self).setUp()
        title = self.config.get('Contact', 'title')
        self.contact = self.gc.open(title)

    def test_properties(self):
        self.assertEqual(self.config.get('Contact', 'id'),
                         self.contact.id)
        self.assertEqual(self.config.get('Contact', 'title'),
                         self.contact.title)

    def test_sheet1(self):
        sheet1 = self.contact.sheet1
        self.assertTrue(isinstance(sheet1, gcontact.Worksheet))

    def test_get_worksheet(self):
        sheet1 = self.contact.get_worksheet(0)
        self.assertTrue(isinstance(sheet1, gcontact.Worksheet))

    def test_worksheet(self):
        sheet_title = self.config.get('Contact', 'sheet1_title')
        sheet = self.contact.worksheet(sheet_title)
        self.assertTrue(isinstance(sheet, gcontact.Worksheet))

    def test_worksheet_iteration(self):
        self.assertEqual(self.contact.worksheets(),
            [sheet for sheet in self.contact])


class WorksheetTest(gcontactTest):

    """Test for gcontact.Worksheet."""

    @classmethod
    def setUpClass(cls):
        super(WorksheetTest, cls).setUpClass()
        title = cls.config.get('Contact', 'title')
        cls.contact = cls.gc.open(title)
    ...
    def setUp(self):
        ...
        self.sheet = self.contact.add_worksheet('wksht_test', 20, 20)

    def tearDown(self):
        self.contact.del_worksheet(self.sheet)
    ...
    def test_acell(self):
        ...
        self.assertTrue(isinstance(cell, gcontact.Cell))
    ...
    def test_range(self):
        ...
        for c1, c2 in zip(cell_range1, cell_range2):
            self.assertTrue(isinstance(c1, gcontact.Cell))
            self.assertTrue(isinstance(c2, gcontact.Cell))
...
class WorksheetDeleteTest(gcontactTest):
    ...
    def setUp(self):
        ...
        self.ws1 = self.contact.add_worksheet(ws1_name, 1, 1)
        self.ws2 = self.contact.add_worksheet(ws2_name, 1, 1)

    def test_delete_multiple_worksheets(self):
        self.contact.del_worksheet(self.ws1)
        self.contact.del_worksheet(self.ws2)


class CellTest(gcontactTest):

    """Test for gcontact.Cell."""
